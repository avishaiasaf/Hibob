 ...
 */
define(['N/action', 'N/query', 'N/record', 'N/search', 'N/task'], /**
 * @param{action} action
 * @param{query} query
 * @param{record} record
 * @param{search} search
 * @param{task} task
 */ (action, query, record, search, task) => {
      const functions = {};
      /**
      ...
      functions.getInputData = (inputContext) => {
            return search.load({
                  id: 'customsearch_hb_arrangement_allocation',
            });
      };
      ...
      functions.map = (mapContext) => {
            ...
            const arngId = values['internalid'].value;
